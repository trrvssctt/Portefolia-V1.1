...
async function init() {
  ...
  // Reset those rows so they are treated as active.
  try {
    const [fixed] = await pool.query(
      "UPDATE utilisateurs SET deleted_at = NULL WHERE deleted_at = '0000-00-00 00:00:00'"
    );
    if (fixed.affectedRows > 0) {
      console.log(`[userModel.init] Fixed ${fixed.affectedRows} user(s) with invalid deleted_at (0000-00-00).`);
    }
  } catch (err) {
    console.warn('userModel.init: could not fix zero-date deleted_at:', err.message);
  }
}
...
async function findByEmail(email) {
  // Always compare lowercased + trimmed so "User@Test.COM" == "user@test.com"
  const normalized = (email || '').trim().toLowerCase();
  if (!normalized) return null;
  const [rows] = await pool.query(
    // deleted_at IS NULL covers normal case; the second condition handles MySQL zero-date
    // '0000-00-00 00:00:00' that can be stored when explicit_defaults_for_timestamp is OFF
    "SELECT * FROM utilisateurs WHERE LOWER(TRIM(email)) = ? AND (deleted_at IS NULL OR deleted_at = '0000-00-00 00:00:00') LIMIT 1",
    [normalized]
  );
  return rows[0] || null;
}
...
async function findByEmailOrPhone(email, phone) {
  const normalized = (email || '').trim().toLowerCase();
  const [rows] = await pool.query(
    "SELECT * FROM utilisateurs WHERE (LOWER(TRIM(email)) = ? OR (phone IS NOT NULL AND phone = ?)) AND (deleted_at IS NULL OR deleted_at = '0000-00-00 00:00:00') LIMIT 1",
