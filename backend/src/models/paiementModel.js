...
async function findById(id) {
  const [rows] = await pool.query(`
    SELECT p.*,
           ...
           COALESCE(
             NULLIF(TRIM(CONCAT(COALESCE(uc.prenom,''), ' ', COALESCE(uc.nom,''))), ''),
             NULLIF(TRIM(CONCAT(COALESCE(ua.prenom,''), ' ', COALESCE(ua.nom,''))), ''),
             'Client supprimé'
           ) AS user_name,
           COALESCE(uc.email, ua.email) AS user_email,
           pl.name AS plan_nom
    FROM paiements p
...
async function list({ page = 1, limit = 50, status = null, user_id = null, from = null, to = null } = {}) {
  ...
  if (user_id) {
    where.push('(c.utilisateur_id = ? OR a.utilisateur_id = ?)');
    params.push(Number(user_id), Number(user_id));
  }
  if (from) { where.push('paiements.created_at >= ?'); params.push(from); }
  if (to) { where.push('paiements.created_at <= ?'); params.push(to); }
...
LEFT JOIN utilisateurs uc ON uc.id = c.utilisateur_id
LEFT JOIN abonnements a   ON a.id  = paiements.abonnement_id
LEFT JOIN utilisateurs ua ON ua.id = a.utilisateur_id
LEFT JOIN plans pl        ON pl.id = a.plan_id
${whereSql}
