...
async function create(req, res) {
  ...
  try {
    ...
    // Detect Business role from JWT (members don't have a paid plan in their own name)
    const jwtRole = (req.userPayload?.role || '').toString().toUpperCase();
    const isBusinessRole = jwtRole === 'BUSINESS_ADMIN' || jwtRole === 'BUSINESS_MEMBER';

    // For business members, get their personal portfolio limit from business_members table
    let businessMemberLimit = Infinity;
    if (jwtRole === 'BUSINESS_MEMBER') {
      const [bmRows] = await pool.query(
        "SELECT portfolio_limit FROM business_members WHERE user_id = ? AND status = 'active' LIMIT 1",
        [userId]
      );
      businessMemberLimit = bmRows[0]?.portfolio_limit ?? 10;
    }
    ...
    // isFreePlan: no plan at all, or explicit free/gratuit slug — never rely on price_cents to avoid misclassifying promo plans
    const isFreePlan = !isBusinessRole && (planSlug === 'gratuit' || planSlug === 'free');
    const isStarterPlan = !isBusinessRole && ['starter', 'standard'].includes(planSlug);
    const isProPlan = !isBusinessRole && ['professionnel', 'pro', 'professional'].includes(planSlug);
    const isPremiumPlan = !isBusinessRole && ['premium', 'premium_pro', 'premium-plus'].includes(planSlug);
    const isBusinessPlan = isBusinessRole || ['business'].includes(planSlug);
    // If no plan and no business role → default to free behaviour
    const effectivelyFree = isFreePlan || (!isBusinessRole && !latestPlan && !isStarterPlan && !isProPlan && !isPremiumPlan && !isBusinessPlan);
    console.log(`User ${userId} creating portfolio with plan slug='${planSlug}' jwtRole=${jwtRole} isBusinessPlan=${isBusinessPlan}`);

    // If free plan (or no plan), ensure user doesn't already have a portfolio (limit 1)
    if (effectivelyFree && !isBusinessPlan) {
    ...
    // Business member: enforce portfolio_limit from business_members table
    if (jwtRole === 'BUSINESS_MEMBER') {
      const existing = await portfolioModel.findByUser(userId);
      const count = existing ? existing.length : 0;
      if (count >= businessMemberLimit) {
        await conn.rollback();
        conn.release();
        console.warn(`[portfolioController.create] business member limit reached for user ${userId} (count=${count} limit=${businessMemberLimit})`);
        return res.status(400).json({ error: `Limite Business atteinte (${businessMemberLimit} portfolios maximum)` });
      }
    }
    ...
    // Enforce social links limit: free=1, starter=3, premium+business=5, pro=unlimited
    ...
    if (effectivelyFree) socialLimit = 1;
    ...
    if (providedSocials > socialLimit) {
      await conn.rollback();
      conn.release();
      const planLabel = effectivelyFree ? 'gratuit' : isStarterPlan ? 'Starter' : isPremiumPlan ? 'Premium' : 'Business';
...
async function update(req, res) {
  ...
  if (incoming.domaines !== undefined || incoming.domain !== undefined || incoming.domaine !== undefined || incoming.domain_enum !== undefined) {
    ...
    if (mappedDomain) mappedUpdate.domaines = mappedDomain;
      ...
      const updateJwtRole = (req.userPayload?.role || '').toString().toUpperCase();
      const updateIsBusinessRole = updateJwtRole === 'BUSINESS_ADMIN' || updateJwtRole === 'BUSINESS_MEMBER';
      const userPlans = await planModel.listUserPlans(userId);
      const latestPlan = userPlans && userPlans.length ? userPlans[0] : null;
      const planSlug = latestPlan && latestPlan.slug ? String(latestPlan.slug).trim().toLowerCase() : 'gratuit';
      console.log(`User ${userId} updating portfolio with plan slug='${planSlug}' jwtRole=${updateJwtRole}`);
      const isFree = !updateIsBusinessRole && (planSlug === 'gratuit' || planSlug === 'free');
      const isStarter = !updateIsBusinessRole && ['starter', 'standard'].includes(planSlug);
      const isPro = !updateIsBusinessRole && ['professionnel', 'pro', 'professional'].includes(planSlug);
      const isPremium = !updateIsBusinessRole && ['premium', 'premium_pro', 'premium-plus'].includes(planSlug);
      const isBusiness = updateIsBusinessRole || ['business'].includes(planSlug);
      const effectivelyFreeUpdate = isFree || (!updateIsBusinessRole && !latestPlan && !isStarter && !isPro && !isPremium && !isBusiness);
      ...
      // Free=1, Starter=3, Premium+Business=5, Pro=unlimited
      let socialLimitUpdate = Infinity;
      if (effectivelyFreeUpdate) socialLimitUpdate = 1;
      else if (isStarter) socialLimitUpdate = 3;
      else if (isPremium || isBusiness) socialLimitUpdate = 5;
      if (provided > socialLimitUpdate) {
        await conn.rollback();
        conn.release();
        const planLabelUpdate = effectivelyFreeUpdate ? 'gratuit' : isStarter ? 'Starter' : isPremium ? 'Premium' : 'Business';
        return res.status(400).json({ error: `Plan ${planLabelUpdate} : vous ne pouvez ajouter que ${socialLimitUpdate} lien(s) de réseaux sociaux.` });
      }
...
async function listAdmin(req, res) {
  try {
    ...
    // attach owner_plan info to each row and add aliases for frontend compatibility
    for (const r of rows) {
      ...
      // Add aliases for frontend compatibility (same as findById)
...
async function getPublicBySlug(req, res) {
  try {
    ...
    // Enrichir avec les infos de l'entreprise si le propriétaire est Business
    const { pool } = require('../db');
    const userId = portfolio.utilisateur_id;

    // Cherche d'abord en tant que membre actif
    const [memberRows] = await pool.query(`
      SELECT bm.role AS member_role, bm.poste,
             ba.company_name, ba.company_logo_url,
             ba.primary_color, ba.secondary_color, ba.accent_color, ba.website_url
      FROM business_members bm
      JOIN business_accounts ba ON ba.id = bm.business_account_id
      WHERE bm.user_id = ? AND bm.status = 'active' AND ba.deleted_at IS NULL
      LIMIT 1
    `, [userId]);

    if (memberRows.length > 0) {
      const biz = memberRows[0];
      portfolio.business = {
        company_name:    biz.company_name,
        company_logo_url: biz.company_logo_url,
        primary_color:   biz.primary_color   || '#1a1a2e',
        secondary_color: biz.secondary_color || '#16213e',
        accent_color:    biz.accent_color    || '#0f3460',
      ...
      const [adminRows] = await pool.query(`
        SELECT ba.company_name, ba.company_logo_url,
               ba.primary_color, ba.secondary_color, ba.accent_color, ba.website_url
        FROM business_accounts ba
        WHERE ba.admin_user_id = ? AND ba.deleted_at IS NULL
        LIMIT 1
      `, [userId]);

      if (adminRows.length > 0) {
        const biz = adminRows[0];
        portfolio.business = {
          company_name:    biz.company_name,
          company_logo_url: biz.company_logo_url,
          primary_color:   biz.primary_color   || '#1a1a2e',
    ...
    // Enregistrer la visite automatiquement à chaque chargement public
    try {
      const ua = req.headers['user-agent'] || '';
      const referer = req.headers['referer'] || req.headers['referrer'] || '';
      const ip = req.headers['x-forwarded-for']?.split(',')[0]?.trim() || req.socket?.remoteAddress || 'unknown';
      await visiteModel.create({ portfolio_id: portfolio.id, adresse_ip: ip, user_agent: ua, referer });
    } catch (visitErr) {
      console.warn('Could not record visit:', visitErr.message);
    }
...
async function remove(req, res) {
  try {
    ...
    const [rows] = await pool.query(
      "SELECT id, utilisateur_id FROM portfolios WHERE id = ? AND (deleted_at IS NULL OR deleted_at = '0000-00-00 00:00:00') LIMIT 1",
      [id]
    );
    const portfolio = rows[0];
    if (!portfolio) return res.status(404).json({ error: 'Portfolio introuvable' });

    if (String(portfolio.utilisateur_id) !== String(userId)) return res.status(403).json({ error: 'Forbidden' });

    // Soft-delete
    await pool.query('UPDATE portfolios SET deleted_at = NOW() WHERE id = ?', [id]);
...
module.exports = { create, update, remove, listByUser, listAdmin, getPublicBySlug, recordVisit, recordVisitBySlug, getById };
