...
async function confirmCheckout(req, res) {
  try {
    ...
    // create user_plan record so PlanContext picks up the active plan
    if (ab.plan_id) {
      try {
        await planModel.subscribeUser({
          utilisateur_id: ab.utilisateur_id,
          plan_id: ab.plan_id,
          status: 'active',
          payment_reference: paymentRef,
          end_date: oneYear,
        });
      } catch (e) { console.warn('Could not create user_plan record', e && e.message); }
    }
