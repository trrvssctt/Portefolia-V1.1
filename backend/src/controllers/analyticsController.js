...
async function getAnalytics(req, res) {
  try {
    ...
    // top countries (full list for geo chart)
    const [countriesRows] = await pool.query(
      `SELECT pays AS country, COUNT(*) AS cnt FROM visites WHERE portfolio_id = ? AND date_visite >= ? AND pays IS NOT NULL AND pays != '' GROUP BY country ORDER BY cnt DESC LIMIT 10`,
      [portfolioId, startDate]
    );
    const topCountry = countriesRows && countriesRows[0] ? countriesRows[0].country || '' : '';
    const allCountries = (countriesRows || []).map(r => ({ country: r.country || 'Inconnu', count: Number(r.cnt) }));
    ...
    // recent visits for live sessions panel (last 10 regardless of period)
    const [recentRaw] = await pool.query(
      `SELECT adresse_ip, user_agent, pays AS country, date_visite FROM visites WHERE portfolio_id = ? ORDER BY date_visite DESC LIMIT 10`,
      [portfolioId]
    );
    const nowMs = Date.now();
    const liveSessions = recentRaw.map(v => {
      const ua = (v.user_agent || '').toLowerCase();
      let device = 'Desktop';
      if (/mobile|android|iphone|ipod/.test(ua)) device = 'Mobile';
      else if (/tablet|ipad/.test(ua)) device = 'Tablette';
      const diffSec = Math.floor((nowMs - new Date(v.date_visite).getTime()) / 1000);
      let time_ago;
      if (diffSec < 60) time_ago = `il y a ${diffSec}s`;
      else if (diffSec < 3600) time_ago = `il y a ${Math.floor(diffSec / 60)}min`;
      else if (diffSec < 86400) time_ago = `il y a ${Math.floor(diffSec / 3600)}h`;
      else time_ago = `il y a ${Math.floor(diffSec / 86400)}j`;
      return { country: v.country || 'Inconnu', device, time_ago };
    });
    ...
    try {
      const [clickRows] = await pool.query(
        `SELECT COUNT(*) AS cnt FROM analytics_events WHERE portfolio_id = ? AND type = 'click' AND created_at >= ?`,
        [portfolioId, startDate]
      );
      totalClicks = clickRows && clickRows[0] ? Number(clickRows[0].cnt) : 0;
